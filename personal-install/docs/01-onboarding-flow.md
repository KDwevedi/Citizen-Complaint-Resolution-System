...
- **digit-ui at `:16080` defaults to the seeded tenant root.** On a Nairobi-flavoured install with `PERSONAL_TENANT_ROOT=ke`, ansible's pre-seed task patches `local-setup/nginx/globalConfigs.js` so `STATE_LEVEL_TENANT_ID="ke"` and `LOCALE_REGION="KE"` — the docker `digit-ui` container bind-mounts that file. Without this patch, the upstream default of `pg/IN` (Punjab/India) would persist. See `05-branding-and-logos.md` for the bind-mount mechanics.
