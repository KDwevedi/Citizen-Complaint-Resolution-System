...
test("scalar, prior, percent normalization, delta modes, and threshold boundaries stay exact", () => {
  ...
  assert.equal(adapters.formatDeltaDisplay(1, "hoursDays", "days"), "▲ 1 DASHBOARD_UNIT_DAY");
...
test("format and ABAC error-label boundaries remain stable", () => {
  ...
  assert.equal(adapters.errorLabel("pii_forbidden"), "DASHBOARD_TILE_ERR_RESTRICTED");
  assert.equal(adapters.errorLabel("kpi_forbidden"), "DASHBOARD_TILE_ERR_NO_ACCESS");
  assert.equal(adapters.errorLabel("scope_forbidden"), "DASHBOARD_TILE_ERR_OUT_OF_SCOPE");
